...
void moveCursor(int x, int y) {
  ...
  a = (unsigned char)(y+32);
  b = (unsigned char)(x+32);
  ...
  if(
    ...
    if(parseCursorLocation(&origx, &origy)) {
      ...
      if(columns != NULL && lines != NULL) {
        ...
        printf("TERMINAL WIDTH (80)?\n");
        d_fgets(&columns, stdin);

        printf("TERMINAL HEIGHT (0)?\n");
        d_fgets(&lines, stdin);
